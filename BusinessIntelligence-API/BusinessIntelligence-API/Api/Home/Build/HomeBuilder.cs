    ...
    public class HomeBuilder
    {
        ...
        public async Task<ComparativoView> GetComparativo(Request req)
        {
            var response = await (from vendas in _context.Vendas
                                  ...
                                  select new
                                  {
                                      valorTotal = (double)g.Sum(x => x.ValorTotal),
                                      valorTotalAnoPassado = (double)g.Sum(x => x.ValorTotalAnoPassado)

                                  }).Select(x => new ComparativoView
                                  {
                                        valorTotal = x.valorTotal,
                                        valorTotalAnoPassado = x.valorTotalAnoPassado,
                                        crescimento = Math.Round((x.valorTotal - x.valorTotalAnoPassado) / x.valorTotalAnoPassado * 100, 2).ToString() + "%"
                                    
                                  }).FirstOrDefaultAsync();

            if (response == null)
                return response = new ComparativoView();
            return response;
        }
    }
}
